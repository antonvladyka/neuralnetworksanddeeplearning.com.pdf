...
\begin{center}
	\includegraphics[width=0.32\linewidth]{figures/ch4/tikz421}
	\begin{tikzpicture}[declare function = {step(\x,\s,\t)=0.5*(sign(\x-\s)-sign(\x-\t));twobumps(\x,\s,\t,\h,\ss,\tt,\hh) = \h*step(\x,\s,\t)+\hh*step(\x,\ss,\tt); }]
	\begin{axis}[width=0.32\linewidth,height=0.4\linewidth,align=center,title={Weighted output\\$s_1=0.4,s_2=0.6,h_1=-1.2$\\$s_1=0.7,s_2=0.9,h_2=0.3$},axis x line=bottom,axis y line=left,xtick={0,1},ytick={-1,0,1},xmax=1.05,ymax=1.25,ymin=-1.25]
	\addplot[blue!90!red,domain=0:1,samples=201]{twobumps(x,0.4,0.6,-1.2,0.7,0.9,0.3};
	\end{axis}
	\end{tikzpicture}
	\begin{tikzpicture}[declare function = {step(\x,\s,\t)=0.5*(sign(\x-\s)-sign(\x-\t));twobumps(\x,\s,\t,\h,\ss,\tt,\hh) = \h*step(\x,\s,\t)+\hh*step(\x,\ss,\tt); }]
	\begin{axis}[width=0.32\linewidth,height=0.4\linewidth,align=center,title={Weighted output\\$s_1=0.2,s_2=0.5,h_1=0.75$\\$s_1=0.7,s_2=0.95,h_2=-0.9$},axis x line=bottom,axis y line=left,xtick={0,1},ytick={-1,0,1},xmax=1.05,ymax=1.25,ymin=-1.25]
	\addplot[blue!90!red,domain=0:1,samples=201]{twobumps(x,0.2,0.5,0.75,0.7,0.95,-0.9};
	\end{axis}
	\end{tikzpicture}
\end{center}
...
\begin{axis}[width=0.42\linewidth,height=0.65\linewidth,align=center,title={Weighted output},axis x line=middle,axis y line=left,xtick={0,1},ytick={-1,0,1},xmax=1.05,ymax=2.2,ymin=-1.5]
	...
	\end{axis}
\end{tikzpicture}	
\end{center}You can see that there are five pairs of hidden neurons. The step points for the respective pairs of neurons are 0,1/5, then 1/5,2/5, and so on, out to 4/5,5/5. These values are fixed - they make it so we get five evenly spaced bumps on the graph.
...
\begin{center} 
	\begin{tikzpicture}
	\begin{axis}[axis lines=middle,width=0.65\linewidth,height=0.5\linewidth,ymin=-2.05,ymax=2.25,scaled y ticks = false,y tick label style={/pgf/number format/fixed},xmin=-0.05,xmax=1.05, xtick={0,1}]
