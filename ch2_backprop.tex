...
%
%Next, we'll prove (\ref{eq:BP2}), which gives an equation for the error $\delta^l$ in terms of the error in the next layer, $\delta^{l+1}$. To do this, we want to rewrite \delta^l_j=\partial{}C/\partial{}z^l_j in terms of $\delta^{l+1}$k=\partial{}C/\partial{}zl+1k. We can do this using the chain rule,
...
%where in the last line we have interchanged the two terms on the right-hand side, and substituted the definition of $\delta^{l+1}$k. To evaluate the first term on the last line, note that
...
%\delta^l_j=∑k$w^{l+1}$kj$\delta^{l+1}$k$\sigma$′(z^l_j).(45)
...
%
%Exercise
%Prove Equations (\ref{eq:BP3}) and (\ref{eq:BP4}).
...
%Output error $\delta^l$: Compute the vector $\delta^l$=∇aC⊙$\sigma$′(zL).
%Backpropagate the error: For each l=L-1,L-2,…,2 compute $\delta^l$=(($w^{l+1}$)T$\delta^{l+1}$)⊙$\sigma$′(zl).
...
%Backpropagate the error: For each l=L-1,L-2,…,2 compute δx,l=(($w^{l+1}$)Tδx,l+1)⊙$\sigma$′(zx,l).
